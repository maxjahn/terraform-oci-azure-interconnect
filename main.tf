...
# the peering ips are chosen arbitrarily, you always can pick other /30s if you do not like the range i used
  cross_connect_mappings {
    oracle_bgp_peering_ip   = "${local.peering_net_prefix}.201/30"
    customer_bgp_peering_ip = "${local.peering_net_prefix}.202/30"
  }

  cross_connect_mappings {
    oracle_bgp_peering_ip   = "${local.peering_net_prefix}.205/30"
    customer_bgp_peering_ip = "${local.peering_net_prefix}.206/30"
  }
}
...
resource "oci_core_service_gateway" "transit_service_gateway" {
  ...
  route_table_id = oci_core_route_table.service_transit_route_table[0].id
}
...
resource "azurerm_express_route_circuit" "connect_erc" {
  ...
  bandwidth_in_mbps     = var.arm_expressroute_bandwidth
